...
public class ImageFetcher
{
	 ...
	 * @return
	 */
	public List<ImageItem> getImagesAllItemList(boolean refresh)
	{
		List<ImageBucket> tmpList = getImagesBucketList(refresh);
		List<ImageItem> imgList=new ArrayList<ImageItem>();
		for(ImageBucket imageBucket:tmpList){
			imgList.addAll(imageBucket.imageList);
		}
		return imgList;
	}
	/**
	 ...
	 * @return
	 */
	public List<String> getImagesAllStringList(boolean refresh)
	{
		List<ImageBucket> tmpList = getImagesBucketList(refresh);
		List<String> imgStrList=new ArrayList<String>();
		for(ImageBucket imageBucket:tmpList){
			for(ImageItem item:imageBucket.imageList){
				imgStrList.add(item.sourcePath);
			}
		}
		return imgStrList;
	}
